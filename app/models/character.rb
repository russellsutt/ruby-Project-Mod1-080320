...
class Character < ActiveRecord::Base
    ...
    def self.talk_to_family
        ...
        if player_selection == 1
            ...
            puts "I am #{Character.first.name}, #{Character.first.titles} of Winterfell. Let me give you some adivce: #{Character.first.quote}".colorize(:light_blue)
            sleep(1)
            talk_to_family
        elsif player_selection == 2
            Catpixel.catpix_high('assets/images/stark/sansa-01.png')
            puts "I am #{Character.second.name}, #{Character.second.titles} of Winterfell. Us Starks need to stick together. #{Character.second.quote}".colorize(:light_blue)
            sleep(1)
            talk_to_family
        elsif player_selection == 3
            Catpixel.catpix_high('assets/images/stark/ned-stark-01.jpeg')
            puts "I am #{Character.third.name}, #{Character.third.titles}.  #{User.last.name}, always be honorable. #{Character.third.quote}".colorize(:light_blue)
            sleep(1)
            talk_to_family
        elsif player_selection == 4
            Catpixel.catpix_high('assets/images/stark/robb-stark-01.jpg')
            puts "I am #{Character.second_to_last.name}, #{Character.second_to_last.titles}. #{User.last.name}, #{Character.second_to_last.quote}".colorize(:light_blue)
            sleep(1)
            talk_to_family
        elsif player_selection == 5
            Catpixel.catpix_high('assets/images/stark/jon-snow-01.jpeg')
            puts "I am #{Character.last.name}, #{Character.last.titles}. #{User.last.name}, always be kind to others. #{Character.last.quote}".colorize(:light_blue)
            sleep(1)
            talk_to_family
